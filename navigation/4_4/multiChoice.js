...
function checkAnswers(myItem){
	...
	for (var i=0;i<questions.length;i++){
		...
		*/
		var wrongAnswers = answerSel.length-$(questions[i]).attr("corrects")*1;
		var temp = qScore_array[i]+$(questions[i]).attr("corrects")*1;
		qScore=(temp<=0?0:temp)/$(questions[i]).attr("corrects")*1;
		score=score+qScore;
	}	
	$("#myScore").val(score);
	foundAll = 0;
	for (var i=0;i<questions.length;i++){
		var answerSel=$(questions[i].getElementsByTagName("input"))
		var answerText=$(questions[i].getElementsByTagName("span"))
		if (foundThem[i]==$(questions[i]).attr("corrects")*1){
			foundAll++;
			for (var j=0;j<answerSel.length;j++){
	...
	for (i=0;i<exItems.length;i++)
	{
		...
		for (j=0;j<y.length;j++)
		{
			...
			//if (countCorrect>1){
				...
				/*if ((foundThem[i]<countCorrect)&&(cookieSavedNo==y.length-countCorrect)){
					foundThem[i] = true;
				}*/
			/*}
			else{
			...
			if ((exJSON.exercise.checkButton)&&(exJSON.exercise.checkButton=="true")){
				theJSONElements += '<li '+'class="animated fadeIn animationDelay1s column'+j+myClass+'">'+'<input name="mul'+i+'" type="'+itemType+'"' + ' onclick=hideAnswers()';
			}
			else {
				theJSONElements += '<li '+'class="animated fadeIn animationDelay1s column'+j+myClass+'">'+'<input name="mul'+i+'" type="'+itemType+'"' + ' onclick=checkAnswers(this)';
			}
			for (var k=0;k<countCorrect;k++){
				if (o[k]==j){
			...
			theJSONElements += ' class="hideMe" />'+'<span'+' onclick=checkAnswersTxt(this)><div class="invisible">'+ y[j]+'</div><img class="mymark correct '+markCorrClass+'" src="true.png" /><img class="mymark wrong '+markWronClass+'" src="false.png" /><img class="mymark" src="option_'+j+'.png" /></span>'+'</li>';
...
function phrasesAppear(){
	...
	for (var i=0;i<x.length;i++){
		...
		for (var j=0;j<y.length;j++){			
			...
			$(z[j].parentNode).addClass('animated fadeInLeft');		
		}
...
function makePageChanges(theContent){
	...
	//PrefixedEvent(document.querySelector("#barrel"), "AnimationEnd", function(){phrasesAppear()});
	//if (cookieSavedNo>0){		
		var x=$(".TrueFalse")
		//alert("cookiJSON = "+JSON.stringify(cookieJSON))
		for (var i=0;i<x.length;i++){
		...
		for (var i=0;i<x.length;i++){
			var y=$(x[i].getElementsByTagName("input"));
			var z=$(x[i].getElementsByTagName("span"));
			if (foundThem[i]==$(x[i]).attr("corrects")*1){
				for (var j=0;j<y.length;j++){
	...
	//}
	//callorig(call1());
	//$("#barrel").addClass('animated fadeInUpBig');	
}
