	...
	for (i=0;i<exItems.length;i++)
	{
		...
		for (j=0;j<y.length;j++)
		{
			...
			if ((exJSON.exercise.checkButton)&&(exJSON.exercise.checkButton=="true")){
				theJSONElements += '<li '+'class="animated rollIn column'+j+myClass+'">'+'<input name="mul'+i+'" type="'+itemType+'"' + ' onclick=hideAnswers()';
			}
			else {
				theJSONElements += '<li '+'class="animated rollIn column'+j+myClass+'">'+'<input name="mul'+i+'" type="'+itemType+'"' + ' onclick=checkAnswers(this)';
			}
			for (var k=0;k<countCorrect;k++){
				if (o[k]==j){
