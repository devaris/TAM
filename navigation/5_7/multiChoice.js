	...
	for (i=0;i<exItems.length;i++)
	{
		...
		for (j=0;j<y.length;j++)
		{
			...
			if ((exJSON.exercise.checkButton)&&(exJSON.exercise.checkButton=="true")){
				theJSONElements += '<li '+'class="column'+j+myClass+' animated rollIn">'+'<input name="mul'+i+'" type="'+itemType+'"' + ' onclick=hideAnswers()';
			}
			else {
				theJSONElements += '<li '+'class="column'+j+myClass+' animated rollIn">'+'<input name="mul'+i+'" type="'+itemType+'"' + ' onclick=checkAnswers(this)';
			}
			for (var k=0;k<countCorrect;k++){
				if (o[k]==j){
			...
			theJSONElements += ' class="hideMe" />'+'<span'+' onclick=checkAnswersTxt(this)><div class="invisible">'+ y[j]+'</div><img class="mymark correct '+markCorrClass+'" src="correct.png" /><img class="mymark wrong '+markWronClass+'" src="wrong.png" /><img class="mymark" src="option_'+j+'.png" /></span>'+'</li>';
