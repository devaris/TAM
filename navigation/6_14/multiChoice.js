...
function readJSON() {
	...
	for (i=0;i<exItems.length;i++)
	{
		...
		theJSONElements += '<div  id="shadow"><img src="shadow.png" alt="shadow" name="shadow" /></div>';
...
function phrasesAppear(){
	...
	for (var i=0;i<x.length;i++){
		...
		for (var j=0;j<y.length;j++){			
			...
			$(z[j].parentNode).addClass('animated fadeIn center');		
		}
		
	}	
	$("#shadow").removeClass("hideMe");
	$("#shadow").addClass("animated fadeIn center");
}

function makePageChanges(theContent){
	myHighlight = 0;
	$("#truefalses").html(theContent);
	PrefixedEvent(document.querySelector("#rubric"), "AnimationEnd", function(){phrasesAppear()});
	//if (cookieSavedNo>0){		
		var x=$(".TrueFalse")
		//alert("cookiJSON = "+JSON.stringify(cookieJSON))
		for (var i=0;i<x.length;i++){
